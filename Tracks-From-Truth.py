...
def parser(left=1000 ,  right= 2000):
    for file in range(left , right) : 
        ...
        how_many_part = truth_file["particle_id"].nunique()
        
        Training_data  = pd.DataFrame(np.nan , index=range(how_many_part) , columns=column_names)  
        appending_index = 0
        start  = time.time()
        ...
        for (_, hits_row), (_, true_row) in zip(hits_file.iterrows(), truth_file.iterrows()): 
            ...
            shape  = Training_data.loc[Training_data["particle_id"] == true_row["particle_id"]].shape[0]
            if shape != 0 : 
                row_values = Training_data.loc[Training_data["particle_id"] == true_row["particle_id"]].values[0]
            ...
            else :# if this if works , then its the first time we see the particl
        ...
        Training_data = Training_data[Training_data["particle_id"] != 0]  # Remove rows where particle_id is 0
